...
echo "-- workspace .venv (/workspaces/grocy/.venv) --"
if [ -d "/workspaces/grocy/.venv" ]; then
  ls -la /workspaces/grocy/.venv | sed -n '1,200p'
  if [ -x "/workspaces/grocy/.venv/bin/python" ]; then
    /workspaces/grocy/.venv/bin/python -m pip --version || echo "workspace venv pip: not available"
  else
