...
from .const import ATTR_CHORES, CHORES, DOMAIN
...
async def async_setup_entry(
    ...
    # Only create buttons if chores are available
    if ATTR_CHORES in coordinator.available_entities:
        # Read current chores data and create a button per chore
        chores_data = coordinator.data.get(ATTR_CHORES) or []
        ...
        for chore in chores_data:
            ...
            entity = GrocyButtonEntity(coordinator, description, config_entry, chore_id)
...
def _extract_chore_fields(item) -> tuple[int | None, str]:
    ...
    # Watch for coordinator updates and create new chore buttons if new
    # chores appear. Do not remove entities automatically to avoid deleting
    # user-registered entities unexpectedly.
    async def _handle_coordinator_update(_: object) -> None:
        ...
        existing_chore_entities = {
            ...
            if entity.entity_description.key.startswith("chore_button_")
        }
            ...
            for removed_id in to_remove_ids:
                ...
                if safe_to_remove:
                    try:
                        ...
                        _LOGGER.debug(
                            "Failed to remove entity %s for chore %s",
                            entity.entity_id,
                            removed_id,
                        )
                    try:
                        coordinator.entities.remove(entity)
                    except ValueError:
                        pass

    # Register a weak-callback to coordinator updates
